...
public class BinMinHeapExemplo
{
	...
	/* Constrói heap vazio. */
	public BinMinHeapExemplo(int tamanho)
	{
		n = 0;
		tam = tamanho;
	...
	public BinMinHeapExemplo(int tamanho, int[] v)
	{
		tam = tamanho;
		vetor = new int[tamanho+1];
		n = tamanho;
	...
	private void refaz(int i)
	{
		...
		while(2*i <= n)
		{
			...
			if((filhoEsq != n) && (vetor[ filhoDir ] < vetor[ filhoEsq ]))
				menorFilho = filhoDir;
